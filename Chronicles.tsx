                ...
                [...genesisPatches].reverse().map(patch => (
                    <div key={patch.id} className="text-sm flex gap-3 p-2 bg-slate-800/50 rounded-md" title={`КОД:\n\n${patch.code}`}>
                        ...
                        <div>
                            ...
                            <span className="font-data text-xs text-slate-500">{new Date(patch.timestamp).toLocaleString('ru-RU')}</span>
                        </div>
                    </div>
                ))
            )}
...
const CrystalArchive: React.FC<Omit<ChroniclesProps, 'eventLog' | 'genesisPatches' | 'logosBits'>> = (props) => {
                        ...
                        <div key={crystal.id} className="glass-panel p-3 rounded-md flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-xs text-slate-400 font-data">
                                    {new Date(crystal.timestamp).toLocaleString('ru-RU')} | Lvl {crystal.stateSnapshot.progression.level}
                                </p>
                            </div>
